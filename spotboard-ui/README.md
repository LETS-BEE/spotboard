...
*   `?r=123`: ID가 123인 런까지 처리하고 정지합니다. (r: run_id)
*   `?t=60`: 대회 시간 60분까지 처리하고 정지합니다. (t: time)

### 키보드 입력을 통한 Manual Feed
`A` 키를 통해 전체 결과를 반영합니다.  
`Space` 키를 통해 자동 제출을 토글할 수 있습니다.  
`N` 키를 통해 알림을 설정할 수 있습니다.  
`F` 키를 통해 하나씩 결과를 공개할 수 있습니다.  


### Freeze 및 Manual Feed
대회 종료 직전(예: 종료 1시간 전)의 상태로 Freeze(동결)된 경우, 새로운 제출 결과는 자동으로 반영되지 않습니다.  
관리자 도구의 "Feed Next" 버튼을 사용하여 하나씩 결과를 공개할 수 있습니다.  

### Award Mode 사용
1.  `/award-editor` 페이지에서 수상 정보를 작성하고 저장합니다.
2.  URL 파라미터 `?award=true` 혹은 `?award=1`를 입력하여 Award Mode를 활성화합니다.
3.  `Space`, `Enter` 또는 `오른쪽 화살표(→)` 키를 눌러 하위 팀부터 순차적으로 결과를 공개합니다.
4.  수상권 팀의 결과가 공개될 때 설정된 수상 정보 슬라이드가 표시됩니다.
5.  이미 결과가 공개된(Finalized) 팀을 클릭하면 해당 팀의 수상 슬라이드를 다시 볼 수 있습니다.
6.  `Esc` 키를 눌러 Award Mode를 종료할 수 있습니다.
